...
game_type = input("Which game would you like your stats for?  Available values: wars, dr, hide, sg, murder, sky, ctf, drop, ground, build, party, bridge, grav, all, main, bed: ")
...
if game_type not in ["wars", "dr", "hide", "sg", "murder", "sky", "ctf", "drop", "ground", "build", "party", "bridge", "grav", "all", "main", "bed"]:
    print("Error: Invalid game selection.")
    exit()

#if game type is bed, parses the response and prints it in the terminal
if game_type == ("bed"):
    xp = stats.json()["xp"]
    print(f"xp: {xp}")
    ...
    print(f"kills: {kills}")
    beds_destroyed = stats.json()["beds_destroyed"]
    print(f"beds_destroyed: {beds_destroyed}")
